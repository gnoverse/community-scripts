...
# Generate N stress wallets and fund them
WALLET_KEYS=()
for i in $(seq 1 "$N"); do
    wkey="chaos_wallet_${i}"
    WALLET_KEYS+=("$wkey")
...
# Bombard: each wallet hits its assigned RPC in parallel
for i in $(seq 1 "$N"); do
