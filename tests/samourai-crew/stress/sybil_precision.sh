#!/bin/bash
# Sybil precision: N wallets hit a single REMOTE in parallel, but each wallet
...
SUFFIX=$(date +%s)
COUNTER_PKGPATH="gno.land/r/${NAMESPACE}/stress/precision${SUFFIX}"
TMPDIR=$(mktemp -d)
